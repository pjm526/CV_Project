...
def point_crop(crop_img,img):
    ...
    # draw_countours contours
    draw_countours = np.zeros(crop_img.shape,np.uint8)
    cv2.drawContours(draw_countours, [max_contour], 0, (0, 255, 0), 0)
    cv2.drawContours(draw_countours, [hull], 0,(0, 0, 255), 0)
    ...
    all_img = np.hstack((draw_countours, crop_img))
